...
func _init(provided_id:int, turn_manager_ref:TurnManager):
	manager_id = provided_id
...
# PURPOSE: Update the manager's record of a unit's position-- does not actually move the unit
...
# PURPOSE: Update the manager's record of units to include the new unit; Does not actually assign the unit to the manager.
func add_unit_ref(provided_unit:Unit)->void:
	...
	provided_unit.unit_type = manager_id
	add_child(provided_unit)
	assemble_manager_vision()
...
# Overridden by both Player_Manager and NPC_Manager, whilst Trap_Manager will ignore it
...
func assemble_manager_vision()->void:
...
# Gets overriden by NPC_Manager and Player_Manager, ignored by Trap_Manager
# PURPOSE: Mark the location of other manager's units if in view
func scan_all_relations()->void:
	pass

# PURPOSE: Return a dictionary of known tiles; This is what the manager knows about their surroundings
func provide_manager_exploration()->Dictionary[Vector3i, int]:
	return explored_tiles

# PURPOSE: Return a dictionary of visible tiles; This is what the manager can actively see
func provide_manager_vision()->Dictionary[Vector3i, int]:
...
func tile_memory_update_and_check(other_manager_units:Dictionary[Vector3i, Entity], provided_dictionary:Dictionary[Vector3i, Array])->void: 
	for tile in other_manager_units:
		if tile in visible_tiles:
			provided_dictionary[tile] = [other_manager_units.get(tile), 0]
...
# PURPOSE: Returns true if the unit is 'yours' (of the same manager)
func is_this_our_unit(provided_unit:Unit)->bool:
	var turn_manager_id:int = provided_unit.get_manager_id()
	return turn_manager_id == manager_id
...
# PURPOSE: Returns a Dictionary with coordinates keyed to a Unit object of YOUR (this manager's) units
# ADDENDUMN: Do not use this for knowing what manager's units to draw on the GUI. Calling a manager's provide_manager_units() is Omniescent and performs no vision checks to see if they're visible to the player
func provide_manager_units()->Dictionary[Vector3i, Entity]:
...
func get_the_unit_on_this_tile(provided_coordinate:Vector3i) -> Entity:
	return turn_manager.get_unit_on_tile(provided_coordinate)
